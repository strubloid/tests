...
describe("Products", () => {
    ...
    it("Invalid data or empty!", async () => {
        // getting the error message to check
        const errorMessage = "Invalid data or empty!";

        // api url to check
        const apiUrl = "https://fakestoreapi.com/products";

        // creating a variable to capture the error
        let capturedError: Error | null = null;
        try {
            // we mock the fetch to return an empty array, getting into the case of invalid data or empty
            globalThis.fetch = async () => ({ ok: true, json: async () => [] }) as Response;

            // we try to get the data from the api url
            const data: APIProduct[] = await ProductService.fetchData(apiUrl);
